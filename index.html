...
<body>
    ...
    <script>
        ...
        map.on('load', () => {
                    ...
                    map.addLayer({
                        ...
                        paint: {
                            'circle-color': [
                                'step',
                                ['get', 'point_count'],
                                '#4CAF50',  // Small clusters
                                50, '#2196F3',  // Medium clusters
                                100, '#FF9800'  // Large clusters
                            ],
                            'circle-radius': [
                                'step',
                                ['get', 'point_count'],
                    ...
                    map.on('click', 'us-states-fill', (e) => {
                        ...
                        if (stateHospitals.length > 0) {
                            stateHospitals.forEach(hospital => {
                                ...
                                listItem.innerHTML = `<strong>${hospital.hospital_name}</strong><br>${hospital.location}<br>EHR System: ${hospital.ehr_system}`;
            ...
            // Click interaction on the 'us-states-fill' layer
            // map.on('click', (e) => {
            //     const [selectedState] = map.queryRenderedFeatures(e.point, {
            //         layers: ['us-states-fill']
            //     });

            //     if (selectedState) {
            //         const { name, id } = selectedState.properties;
            //         alert(`The ${id} is short for ${name}`);
            //     }
            //     // console.log(selectedState)
            // });

            // Hover effect for the 'us-states-fill' layer
