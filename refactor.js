...
// Global variables to store facilities data and regions/states with facilities
let facilitiesData = [];
...
fetch('./data/facilities.json')
    ...
    .then(facilities => {
        facilitiesData = facilities; // Store fetched data globally
...
//     // Hover behavior for regions with facilities
...
//         // Remove hover state from the previous region if not selected
...
//         // Update the current hovered region
//         hoveredRegionId = regionId;

//         // Apply hover color to the current region if it's not selected
//         if (hoveredRegionId !== selectedRegionId) {
//             map.setFeatureState({ source: sourceName, id: hoveredRegionId }, { hover: true });
//         }
//     });

//     // Remove hover effect when the mouse leaves the region
...
//         // Only allow selection for regions with facilities
//         if (!regionsWithFacilities.has(regionId)) return;
